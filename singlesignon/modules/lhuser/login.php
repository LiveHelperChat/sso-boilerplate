...
if (!$currentUser->isLogged()){
    ...
    try {
        ...
        $ts = time();
        $hash = sha1($UserData->id . '_' . $UserData->password . '_' . erConfigClassLhConfig::getInstance()->getSetting('site','secrethash') . '_' . $ts);
        erLhcoreClassUser::instance()->logout();
        erLhcoreClassModule::redirect('user/loginasuser','/' . $UserData->id . '/(hash)/' . $hash . '/(ts)/' . $ts);
        exit;
